 ...
 * Proyecto: DemoPdf
 * Fichero: DemoPdf.prg
 * Descripci�n: Ejemplo de uso de THaruPdf
 * Autor: Carlos Mora
 * Fecha: 06/01/2017
 */
...
PROCEDURE Main( cFileToSave )
   ...
   ? "Saving to:", cFileToSave

   // Configuracion de los FONTs a utilizar en la impresion.
   GetHaruFontList()
   HaruAddFont( 'Arial Black', 'ariblk.ttf' )
   HaruAddFont( 'i2of5txt', 'i2of5txt.ttf', .T. )

/*
   To have code that can exchange TPrinter and THaru it is important
   very well separate the declaration of the TPrinter object and the sources of the rest
...
*/
   ...
   FOR nOffset := 0 TO 14.85 STEP 14.85
      WITH OBJECT oPrint
         ...
         :CmSay( nOffset + 1.6, 10, 'Codigo 09', oFontMin, , , , 0 )
         ...
         :CmSay( nOffset + 1.0, 15.2, 'NO 0001-00001044', oFontBold )
         ...
         :CmSay( nOffset + 4.4, 1.0, 'SENOR:', oFont )
         :CmSay( nOffset + 4.35, 2.6, 'Baudilio Cuevas Ibaoez', oFontPrint )
         ...
         ENDIF
         :CmSay( nOffset + 7.2, 4.6, 'CON CHEQUE NO', oFontRed )
         IF !Empty( cBanco )
         ...
         // Conpceptos
         :CmSay( nOffset + 10, 1.0, 'Descripcion del cobro', oFontRed )
         ...
         // Observaciones
         aObservaciones := { 'Recuerde que para renovar su credito' ;
            , 'debera presentar su ultimo Recibo de Haberes,' ;
